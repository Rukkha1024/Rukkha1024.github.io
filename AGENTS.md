...
- 변경 후 관련 문서와 페이지 확인.
- 기존 스타일 유지. 최소 범위만 수정.
- CV 정본: resume.lol resume `Minseok`. 로컬 생성기 없음. DOCX 파이프라인 없음.
- CV 작업: `.agents/skills/phd-candidate-cv/` 먼저 읽고 수정.
- CV skill 수정: `.claude/skills/phd-candidate-cv/` 전체 tree byte-identical mirror.
- CV 입력·증명서·스냅샷: `local/` 또는 ignore 경로만 사용. 공개 금지. share link는 사용자 승인 필수.
...
- `local/cv/`: resume.lol CV 스냅샷. Git 제외.
- `files/Minseok_Cho_Academic_CV_*`: CV 내보내기 자리. Git 제외.
