...
3. Treat `.agents/skills/phd-candidate-academic-cv/` as canonical. Copy its
   complete file tree to `.claude/skills/phd-candidate-academic-cv/` after each
   skill change. Require matching relative paths and byte-identical files.
4. Refuse a CV-only edit. The source JSON's `contract_version` and the DOCX
   identifier must match the current instruction version.
5. Keep personal values out of tracked skill files. Store them only in an
...
Run `quick_validate.py` on both skill directories, the skill unit tests, mirror
byte checks, `git diff --check`, DOCX/PDF audits, and full render review. Stage
explicit tracked paths only. Commit and push the instruction checkpoint before
generating the CV; commit and push any later tracked privacy or workflow change
separately.
